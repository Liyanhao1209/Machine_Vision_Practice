...
cv2.waitKey(0)
# 0表示纯黑，255表示纯白
ret, th1 = cv2.threshold(gray_img, 245, 255, cv2.THRESH_BINARY)
cv2.imshow('Threshold', th1)
cv2.waitKey(0)


dilation = cv2.dilate(th1, kernel, iterations=2)  # 膨胀
cv2.imshow('Dilation', dilation)
cv2.waitKey(0)
dist_img = cv2.distanceTransform(dilation, cv2.DIST_L2, cv2.DIST_MASK_5)  # 距离变换
cv2.imshow('DistanceTransformation', dist_img)
cv2.waitKey(0)
dist_output = cv2.normalize(dist_img, 0, 1.0, cv2.NORM_L2SQR)  # 归一化
